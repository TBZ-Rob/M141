# 🗄️ M141 – Datenbanksystem in Betrieb nehmen

![Modul](https://img.shields.io/badge/Modul-M141-blue)
![Klasse](https://img.shields.io/badge/Klasse-PE24c-orange)
![Autor](https://img.shields.io/badge/Autor-Robin%20Nydegger-lightgrey)
![Zeitraum](https://img.shields.io/badge/Zeitraum-Mai–Juli%202025-lightgrey)

---

## ⚠️ Disclaimer

Dieses Repository ist ein **persönliches Lernportfolio** im Rahmen des Moduls M141 an der TBZ Zürich.

Die Inhalte wurden eigenständig erarbeitet. Für folgende Bereiche wurde **Claude AI Pro** (Anthropic) als Hilfsmittel eingesetzt:

- 📐 Gestaltung und Struktur des Repositories
- ✏️ Rechtschreibung und Formulierungen
- 📊 Diagramme und visuelle Darstellungen
- 🤖 Claude hat über den **Filesystem MCP Server** direkten Lese- und Schreibzugriff auf dieses Repository

Alle verwendeten Prompts sind auf der Seite [Prompts](./Lernportfolio/Prompts.md) dokumentiert.



---

## 📁 Inhaltsverzeichnis

- [📚 Lernportfolio](./Lernportfolio/README.md)
- [📂 Files / SQL-Scripts](./files/)

---

## 📝 Beurteilungen

| LB | Gewicht | Datum | Themen |
|----|---------|-------|--------|
| **LB1** | 20% | 09.06. | Tag 1–4 |
| **LB2** | 30% | 23.06. | Tag 1–7 |
| **[LB3](./Lernportfolio/LB3-Praxisarbeit/README.md)** | 50% | 07.07. | Praxisarbeit + Portfolio |

---

$\textcolor{#8b949e}{\text{Hinweis: Diagramme, Rechtschreibung und Repo-Struktur wurden mit }} \textcolor{#D4622A}{\text{Claude AI Pro}} \textcolor{#8b949e}{\text{ generiert.}}$

<a href="./Lernportfolio/Prompts.md" style="color:#D4622A;">Prompts</a>
